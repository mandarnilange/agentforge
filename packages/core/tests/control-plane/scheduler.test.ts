...
describe("LocalAgentScheduler", () => {
	it("schedules an agent with no required capabilities to any node", async () => {
		const scheduler = new LocalAgentScheduler();
		const agent = makeAgent("analyst");
		const node = await scheduler.schedule(agent, [LOCAL_NODE]);
		expect(node.metadata.name).toBe("local");
	});

	it("schedules agent requiring docker to node with docker", async () => {
		const scheduler = new LocalAgentScheduler();
		const agent = makeAgent("developer", ["docker"]);
		const node = await scheduler.schedule(agent, [LOCAL_NODE, DOCKER_NODE]);
		expect(node.metadata.name).toBe("local-docker");
	});

	it("throws when no node satisfies required capabilities", async () => {
		const scheduler = new LocalAgentScheduler();
		const agent = makeAgent("developer", ["docker"]);
		await expect(scheduler.schedule(agent, [LOCAL_NODE])).rejects.toThrow(
			/no available node/i,
		);
	});

	it("throws when all nodes are at capacity", async () => {
		const scheduler = new LocalAgentScheduler();
		const agent = makeAgent("analyst");
		...
		await expect(scheduler.schedule(agent, [fullNode])).rejects.toThrow(
			/no available node/i,
		);
	});

	it("respects maxConcurrentRuns and picks node with capacity", async () => {
		const scheduler = new LocalAgentScheduler();
		const agent = makeAgent("analyst");
		...
		const node = await scheduler.schedule(agent, [full, free]);
		expect(node.metadata.name).toBe("free");
	});

	it("tracks active runs and decrements on completion", async () => {
		const scheduler = new LocalAgentScheduler();
		scheduler.recordRunStarted("local");
		scheduler.recordRunStarted("local");
		expect(await scheduler.getActiveRunCount("local")).toBe(2);

		scheduler.recordRunCompleted("local");
		expect(await scheduler.getActiveRunCount("local")).toBe(1);
	});

	it("prefers node matching preferred capabilities", async () => {
		const scheduler = new LocalAgentScheduler();
...
describe("LocalAgentScheduler without registry", () => {
	it("schedules normally when registry not provided", async () => {
		const scheduler = new LocalAgentScheduler();
		const agent = makeAgent("analyst");
		const node = await scheduler.schedule(agent, [LOCAL_NODE]);
		expect(node.metadata.name).toBe("local");
